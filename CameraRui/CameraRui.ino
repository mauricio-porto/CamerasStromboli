...
IPAddress local_IP(192,168,9,10);
...
static esp_err_t flash_handler(httpd_req_t *req) {
  const char resp[] = "GET Flash Response";
  httpd_resp_send(req, resp, strlen(resp));
  return ESP_OK;
}

static esp_err_t shit_handler(httpd_req_t *req) {
  const char resp[] = "GET Shit Response";
  httpd_resp_send(req, resp, strlen(resp));
  return ESP_OK;
}
...
void startCameraServer(){
  ...
  httpd_uri_t shit_uri = {
    .uri       = "/shit",
    .method    = HTTP_GET,
    .handler   = shit_handler,
    .user_ctx  = NULL
  };
  ...
  if (httpd_start(&server, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(server, &shit_uri);
  }
}
...
void setup() {
  ...
  pinMode(4, OUTPUT);
  digitalWrite(4, LOW);  // HIGH irá ligar
